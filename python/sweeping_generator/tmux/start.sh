...
# ---------------- window: sweeping_generator ----------------
read W_sweeping_generator P <<< "$(tmux new-window -t "$SESSION_NAME" -n "sweeping_generator" -P -F '#{window_id} #{pane_id}')"
tmux send-keys -t "$P" "$SETUP; "'waitForControl; roslaunch pairs_example_sweeping_generator sweeping_generator.launch' Enter
P=$(tmux split-window -t "$W_sweeping_generator" -P -F '#{pane_id}')
tmux select-layout -t "$W_sweeping_generator" tiled
tmux send-keys -t "$P" "$SETUP; "'history -s rosservice call /$UAV_NAME/sweeping_generator/start 3.0' Enter
tmux select-layout -t "$W_sweeping_generator" tiled
...
tmux select-window -t "$W_sweeping_generator"
