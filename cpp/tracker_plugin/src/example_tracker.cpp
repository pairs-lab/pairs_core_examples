...
#include <pairs_example_tracker_plugin/example_trackerConfig.h>

//}

namespace pairs_example_tracker_plugin
{
...
class ExampleTracker : public pairs_uav_managers::Tracker {
public:
...
private:
  ...
  typedef pairs_example_tracker_plugin::example_trackerConfig DrsConfig_t;
  typedef dynamic_reconfigure::Server<DrsConfig_t>      Drs_t;
  boost::shared_ptr<Drs_t>                              drs_;
  void                                                  callbackDrs(pairs_example_tracker_plugin::example_trackerConfig &config, uint32_t level);
...
bool ExampleTracker::initialize(const ros::NodeHandle &nh, std::shared_ptr<pairs_uav_managers::control_manager::CommonHandlers_t> common_handlers,
                                std::shared_ptr<pairs_uav_managers::control_manager::PrivateHandlers_t> private_handlers) {
  ...
  success *= private_handlers->loadConfigFile(ros::package::getPath("pairs_example_tracker_plugin") + "/config/example_tracker.yaml");
  ...
  pairs_lib::ParamLoader param_loader(nh_, "ExampleTracker");

  // This is the alternaive way of loading the config file.
  //
  // Files loaded using this method are prioritized over ROS params.
  //
  // param_loader.addYamlFile(ros::package::getPath("pairs_example_tracker_plugin") + "/config/example_tracker.yaml");
...
std::tuple<bool, std::string> ExampleTracker::activate([[maybe_unused]] const std::optional<pairs_msgs::TrackerCommand> &last_tracker_cmd) {
  ...
  if (last_tracker_cmd) {
    ...
    try {
      heading_ = pairs_lib::AttitudeConverter(uav_state.pose.orientation).getHeading();
    }
    catch (...) {
      heading_ = 0;
    }
...
std::optional<pairs_msgs::TrackerCommand> ExampleTracker::update(const pairs_msgs::UavState &                                          uav_state,
  ...
  double          to_goal_heading  = pairs_lib::geometry::sradians::diff(goal_heading_, heading_);
  ...
  double step_heading = pairs_lib::signum(to_goal_heading) * constraints.heading_speed * dt;
...
const pairs_msgs::TrackerStatus ExampleTracker::getStatus() {
...
const std_srvs::TriggerResponse::ConstPtr ExampleTracker::switchOdometrySource([[maybe_unused]] const pairs_msgs::UavState &new_uav_state) {
...
const pairs_msgs::DynamicsConstraintsSrvResponse::ConstPtr ExampleTracker::setConstraints([
...
const pairs_msgs::ReferenceSrvResponse::ConstPtr ExampleTracker::setReference([[maybe_unused]] const pairs_msgs::ReferenceSrvRequest::ConstPtr &cmd) {
  ...
  pairs_msgs::ReferenceSrvResponse response;

  response.message = "reference set";
  response.success = true;

  return pairs_msgs::ReferenceSrvResponse::ConstPtr(new pairs_msgs::ReferenceSrvResponse(response));
}
...
const pairs_msgs::VelocityReferenceSrvResponse::ConstPtr ExampleTracker::setVelocityReference([
...
const pairs_msgs::TrajectoryReferenceSrvResponse::ConstPtr ExampleTracker::setTrajectoryReference([
...
void ExampleTracker::callbackDrs(pairs_example_tracker_plugin::example_trackerConfig &config, [[maybe_unused]] uint32_t level) {

  pairs_lib::set_mutexed(mutex_drs_params_, config, drs_params_);
...
PLUGINLIB_EXPORT_CLASS(pairs_example_tracker_plugin::example_tracker::ExampleTracker, pairs_uav_managers::Tracker)
