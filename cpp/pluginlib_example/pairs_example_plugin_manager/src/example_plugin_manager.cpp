...
#include <pairs_example_plugin_manager/plugin_interface.h>
...
namespace pairs_example_plugin_manager
{
...
private:
  ...
  std::shared_ptr<pairs_example_plugin_manager::CommonHandlers_t> common_handlers_;

  // | --------------- dynamic loading of plugins --------------- |

  std::unique_ptr<pluginlib::ClassLoader<pairs_example_plugin_manager::Plugin>> plugin_loader_;  // pluginlib loader
  std::vector<std::string>                                                _plugin_names_;
  std::map<std::string, PluginParams>                                     plugins_;      // map between plugin names and plugin params
  std::vector<boost::shared_ptr<pairs_example_plugin_manager::Plugin>>          plugin_list_;  // list of plugins, routines are callable from this
...
void ExamplePluginManager::onInit() {
  ...
  pairs_lib::ParamLoader param_loader(nh_, "ExamplePluginManager");
  ...
  common_handlers_ = std::make_shared<pairs_example_plugin_manager::CommonHandlers_t>();
  ...
  plugin_loader_ = std::make_unique<pluginlib::ClassLoader<pairs_example_plugin_manager::Plugin>>("pairs_example_plugin_manager", "pairs_example_plugin_manager::Plugin");
...
void ExamplePluginManager::timerUpdate([[maybe_unused]] const ros::TimerEvent& event) {
  ...
  auto active_plugin_idx = pairs_lib::get_mutexed(mutex_plugins_, active_plugin_idx_);
...
PLUGINLIB_EXPORT_CLASS(pairs_example_plugin_manager::ExamplePluginManager, nodelet::Nodelet)
