...
#include <pairs_example_waypoint_flier/dynparamConfig.h>
...
namespace pairs_example_waypoint_flier
{
...
private:
  ...
  pairs_lib::SubscribeHandler<nav_msgs::Odometry>                  sh_odometry_;
  pairs_lib::SubscribeHandler<pairs_msgs::ControlManagerDiagnostics> sh_control_manager_diag_;

  void              callbackControlManagerDiag(const pairs_msgs::ControlManagerDiagnostics::ConstPtr msg);
  ...
  std::vector<pairs_msgs::Reference> waypoints_;
  std::string                      _waypoints_frame_;
  bool                             waypoints_loaded_ = false;
  pairs_msgs::Reference              current_waypoint_;
  ...
  typedef pairs_example_waypoint_flier::dynparamConfig                              Config;
  typedef dynamic_reconfigure::Server<pairs_example_waypoint_flier::dynparamConfig> ReconfigureServer;
  boost::recursive_mutex                                                      mutex_dynamic_reconfigure_;
  boost::shared_ptr<ReconfigureServer>                                        reconfigure_server_;
  void                                                                        callbackDynamicReconfigure(Config& config, uint32_t level);
  pairs_example_waypoint_flier::dynparamConfig                                      last_drs_config_;
  ...
  std::vector<pairs_msgs::Reference> matrixToPoints(const Eigen::MatrixXd& matrix);

  void offsetPoints(std::vector<pairs_msgs::Reference>& points, const Eigen::MatrixXd& offset);

  double distance(const pairs_msgs::Reference& waypoint, const geometry_msgs::Pose& pose);
};
...
void ExampleWaypointFlier::onInit() {
  ...
  // | ------------------- load ros parameters ------------------ |
  /* (pairs_lib implementation checks whether the parameter was loaded or not) */
  pairs_lib::ParamLoader param_loader(nh, "ExampleWaypointFlier");
  ...
  sh_odometry_             = pairs_lib::SubscribeHandler<nav_msgs::Odometry>(shopts, "odom_uav_in");
  ...
  pub_dist_to_waypoint_ = nh.advertise<pairs_msgs::Float64Stamped>("dist_to_waypoint_out", 1);
  pub_reference_        = nh.advertise<pairs_msgs::ReferenceStamped>("reference_out", 1);
...
void ExampleWaypointFlier::callbackControlManagerDiag(const pairs_msgs::ControlManagerDiagnostics::ConstPtr diagnostics) {
  ...
  pairs_msgs::Reference current_waypoint = pairs_lib::get_mutexed(mutex_current_waypoint_, current_waypoint_);

  // extract the pose part of the odometry
  geometry_msgs::Pose current_pose = pairs_lib::getPose(sh_odometry_.getMsg());
...
void ExampleWaypointFlier::timerPublishSetReference([[maybe_unused]] const ros::TimerEvent& te) {
  ...
  pairs_msgs::ReferenceStamped new_waypoint;
  ...
  pairs_lib::set_mutexed(mutex_current_waypoint_, waypoints_.at(idx_current_waypoint_), current_waypoint_);
...
void ExampleWaypointFlier::timerPublishDistToWaypoint([[maybe_unused]] const ros::TimerEvent& te) {
  ...
  pairs_msgs::Float64Stamped dist_msg;
...
bool ExampleWaypointFlier::callbackFlyToFirstWaypoint([[maybe_unused]] std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res) {
  ...
  if (waypoints_loaded_) {
    ...
    pairs_lib::set_mutexed(mutex_current_waypoint_, waypoints_.at(0), current_waypoint_);
...
std::vector<pairs_msgs::Reference> ExampleWaypointFlier::matrixToPoints(const Eigen::MatrixXd& matrix) {

  std::vector<pairs_msgs::Reference> points;

  for (int i = 0; i < matrix.rows(); i++) {

    pairs_msgs::Reference point;
...
void ExampleWaypointFlier::offsetPoints(std::vector<pairs_msgs::Reference>& points, const Eigen::MatrixXd& offset) {
...
double ExampleWaypointFlier::distance(const pairs_msgs::Reference& waypoint, const geometry_msgs::Pose& pose) {

  return pairs_lib::geometry::dist(vec3_t(waypoint.position.x, waypoint.position.y, waypoint.position.z),
...
PLUGINLIB_EXPORT_CLASS(pairs_example_waypoint_flier::ExampleWaypointFlier, nodelet::Nodelet);
