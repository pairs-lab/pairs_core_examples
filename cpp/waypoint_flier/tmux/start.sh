...
# ---------------- window: pairs_example_waypoint_flier ----------------
read W_pairs_example_waypoint_flier P <<< "$(tmux new-window -t "$SESSION_NAME" -n "pairs_example_waypoint_flier" -P -F '#{window_id} #{pane_id}')"
tmux send-keys -t "$P" "$SETUP; "'waitForControl; roslaunch pairs_example_waypoint_flier pairs_example_waypoint_flier.launch' Enter
P=$(tmux split-window -t "$W_pairs_example_waypoint_flier" -P -F '#{pane_id}')
tmux select-layout -t "$W_pairs_example_waypoint_flier" tiled
tmux send-keys -t "$P" "$SETUP; "'history -s rosservice call /$UAV_NAME/pairs_example_waypoint_flier/fly_to_first_waypoint' Enter
P=$(tmux split-window -t "$W_pairs_example_waypoint_flier" -P -F '#{pane_id}')
tmux select-layout -t "$W_pairs_example_waypoint_flier" tiled
tmux send-keys -t "$P" "$SETUP; "'history -s rosservice call /$UAV_NAME/pairs_example_waypoint_flier/start_waypoints_following' Enter
P=$(tmux split-window -t "$W_pairs_example_waypoint_flier" -P -F '#{pane_id}')
tmux select-layout -t "$W_pairs_example_waypoint_flier" tiled
tmux send-keys -t "$P" "$SETUP; "'history -s rosservice call /$UAV_NAME/pairs_example_waypoint_flier/stop_waypoints_following' Enter
tmux select-layout -t "$W_pairs_example_waypoint_flier" tiled
...
tmux select-window -t "$W_pairs_example_waypoint_flier"
