...
#include <pairs_example_controller_plugin/example_controllerConfig.h>
...
namespace pairs_example_controller_plugin
{
...
class ExampleController : public pairs_uav_managers::Controller {
...
private:
  ...
  typedef pairs_example_controller_plugin::example_controllerConfig DrsConfig_t;
  typedef dynamic_reconfigure::Server<DrsConfig_t>            Drs_t;
  boost::shared_ptr<Drs_t>                                    drs_;
  void                                                        callbackDrs(pairs_example_controller_plugin::example_controllerConfig& config, uint32_t level);
...
bool ExampleController::initialize(const ros::NodeHandle& nh, std::shared_ptr<pairs_uav_managers::control_manager::CommonHandlers_t> common_handlers,
                                   std::shared_ptr<pairs_uav_managers::control_manager::PrivateHandlers_t> private_handlers) {
  ...
  success *= private_handlers->loadConfigFile(ros::package::getPath("pairs_example_controller_plugin") + "/config/example_controller.yaml");

  if (!success) {
    return false;
  }

  pairs_lib::ParamLoader param_loader(nh_, "ExampleController");
...
void ExampleController::updateInactive(const pairs_msgs::UavState& uav_state, [[maybe_unused]] const std::optional<pairs_msgs::TrackerCommand>& tracker_command) {
...
ExampleController::ControlOutput ExampleController::updateActive(const pairs_msgs::UavState& uav_state, const pairs_msgs::TrackerCommand& tracker_command) {
  ...
  if (common_handlers_->detailed_model_params) {
    ...
    pairs_uav_managers::control_manager::DetailedModelParams_t detailed_model_params = common_handlers_->detailed_model_params.value();
  ...
  position_reference.pose.orientation = pairs_lib::AttitudeConverter(0, 0, 0).setHeading(tracker_command.heading);
  ...
  attitude_cmd.orientation = pairs_lib::AttitudeConverter(drs_params.roll, drs_params.pitch, drs_params.yaw);
  attitude_cmd.throttle    = pairs_lib::quadratic_throttle_model::forceToThrottle(common_handlers_->throttle_model,
  ...
  last_control_output_.desired_orientation = pairs_lib::AttitudeConverter(drs_params.roll, drs_params.pitch, drs_params.yaw);
...
const pairs_msgs::ControllerStatus ExampleController::getStatus() {
...
void ExampleController::switchOdometrySource([[maybe_unused]] const pairs_msgs::UavState& new_uav_state) {
}
...
const pairs_msgs::DynamicsConstraintsSrvResponse::ConstPtr ExampleController::setConstraints([
...
void ExampleController::callbackDrs(pairs_example_controller_plugin::example_controllerConfig& config, [[maybe_unused]] uint32_t level) {
...
PLUGINLIB_EXPORT_CLASS(pairs_example_controller_plugin::example_controller::ExampleController, pairs_uav_managers::Controller)
